...
import 'package:fe/ADMIN/course/add.course.admin.page.dart';
import 'package:fe/ADOANHNGHIEP/course/edit.course.employer.page.dart';
...
class CourseEmployerPage extends StatefulWidget {
  const CourseEmployerPage({super.key});

  @override
  State<CourseEmployerPage> createState() => _CourseEmployerPageState();
}

class _CourseEmployerPageState extends State<CourseEmployerPage> {
  List<CourseModel> listData = [];
  TextEditingController find = TextEditingController();
  String? name;
  getData() async {
    var idDn = await MySP.getIdSV();
    var listDataNew = await CourseProvider.getListForDN(name: name, idDn: idDn);
    setState(() {
                ...
                InkWell(
                  onTap: () async {
                    var data = await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (BuildContext context) => EditCourseEmployerPage(
                          courseModel: widget.element,
                        ),
                      ),
                    );
